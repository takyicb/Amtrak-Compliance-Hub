"""
ingest_privacy_laws.py
Law-aware ingestion for Privacy RAG (Chroma + OpenAI embeddings)

- Splits by statute sections first (jurisdiction-specific markers)
- Then splits long sections by subsections ((a), (b), (1), (2), …)
- Enforces token-ish sizes with overlap while retaining headings
- Stores rich metadata (jurisdiction, law, section_id, section_title, hierarchy_path)
- Adds canonical Signed/Effective dates to every chunk's metadata
- Batches upserts and dedupes by text hash to avoid re-embedding unchanged chunks

Requirements:
  pip install chromadb python-dotenv openai
.env:
  OPENAI_API_KEY=sk-...
  EMBED_MODEL=text-embedding-3-small  # (1536-dim; default if omitted)
"""
...
LAW_DATES_RAW = {
    "California Consumer Privacy Act (as amended)": {"signed": "6/28/2018","effective": "1/1/2020"},
    "Virginia Consumer Data Protection Act": {"signed": "3/2/2021","effective": "1/1/2023"},
    "Delaware Personal Data Privacy Act (2023)": {"signed": "9/11/2023","effective": "1/1/2025"},
    "Illinois Biometric Information Privacy Act (BIPA)": {"signed": "10/3/2008","effective": "10/3/2008"},
    "New Jersey SB 332 (2025)": {"signed": "1/16/2024","effective": "1/15/2025"},
    "New Hampshire SB 255 (2025)": {"signed": "3/6/2024","effective": "1/1/2025"},
}
        ...
        for sub_heading, sub_text, hierarchy in subs:
            ...
            for c_idx, (eff_head, chunk_text, hier) in enumerate(sized, start=1):
                ...
                meta = {
                    ...
                    # store list as JSON string to satisfy Chroma's scalar-only metadata
                    "hierarchy_path": json.dumps(hier, ensure_ascii=False),
...
# ── Metadata sanitizer (defensive) ──────────────────────────────────────────────
def sanitize_metadata(meta: Dict) -> Dict:
    """Ensure all metadata values are str/int/float/bool/None for Chroma."""
...
def upsert_records(records: List[Dict]):
    ...
    for i, group in enumerate(batched(records, BATCH_SIZE), start=1):
        ...
        metas = [sanitize_metadata(r["meta"]) for r in group]  # ← sanitize here
